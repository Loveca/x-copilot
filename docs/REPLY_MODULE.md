...
- **思考模式默认关闭**（`"thinking":{"type":"disabled"}`）。DeepSeek V4 的思考模式**默认开启且 `reasoning_effort=high`**，会在输出正文前先跑一段 `reasoning_content` 思维链；写回复不需要推理，实测这一项就是 12 秒延迟的全部来源（首字节 0.2s → 首条 12.5s → 完成 12.9s）。设置页「模型配置 → 深度思考」可开回
...
- **但如果模型先跑思维链，流式也无能为力**：12 秒里一个正文字符都没有，面板只能干等。所以真正的第一优化项是**关掉思考模式**（本项目已默认关闭），流式是第二优化项
