...
> 本文记录「回复生成」模块截至当前的全部功能边界与实现要点。产品需求见 [PROJECT.md](../PROJECT.md)（仓库根目录），技术选型见 [ARCHITECTURE.md](./ARCHITECTURE.md)，未完成项见 [TODO.md](./TODO.md)。

**状态**：功能已基本完成并实测通过（2026-09-15）——自动/手动触发、意图输入、风格配置、流式生成、带图理解、缓存与填入全部验证可用；首条候选 DeepSeek 约 0.8s、Gemini 约 1.2s。剩余项集中在 §10 的增强与 `TODO.md` 的历史 bug。
...
- **帖子图片（自动带）**：抓推文 DOM 里 `pbs.twimg.com/media/` 下的照片附件，最多 4 张，统一归一化成 `?format=jpg&name=small`（约 680px、几十 KB）。
  ...
  - ⚠️ **纯图帖（无正文）必须视为有效帖子**：检测判空条件是「有文字 **或** 有图片」，不能只判文字，否则这类帖子根本不弹面板。面板对无正文帖显示占位文案，prompt 里也明确告知模型「这条没有文字」，避免它自行编造内容
  - 视频封面按设计不带（首帧常与内容无关，易让模型答偏）
