...
> - **「填入时闪一下」→ 已回退**（同日二次，用户反馈 bug 大致修好后剩余此症状）：
...
>
>   **⚠️ 但 026d502 引入了回归（2026-09-17 11:00 用户实测）——已按用户要求暂停改动，只记录、不再改**：
>   - **症状**：① **退格又删不掉了**（S2 复现）；② **填入看似替换成功，但 X 的字符计数仍在累计**
>     （= 编辑器状态里是「旧内容 + 新内容」的追加，不是替换）。
>   - **根因（可从上一步 diff 直接推出）**：`document.execCommand('selectAll')` 只移动 **DOM 选区**，
>     **不会同步 DraftJS 的 `EditorState.selection`**。所以：
>     ① 去掉显式清空（`selectAll` + `delete`）后，**paste 在编辑器状态里是「在末尾追加」而不是替换**
>        —— 字符计数累计正是这个的直接证据；
>     ② 新加的「宽限接受」（文字结构对得上就判成功）把这条追加结果直接当成成功，
>        **不再降级到原本真正管用的 `2-insertText`**，等于把唯一有效的路径也关掉了。
>   - **由此确立的两条硬结论**：
>     1. **在该 composer 上不能假设「全选 + paste = 替换」**；清空必须走 `selectAll` + `delete`
>        （这条此前是可靠的），写入用 `insertText` 替换选区或「先清后写」。
>     2. **按钮判据不亮时必须继续降级**，不能用"文字看着对"来兜底接受 —— 那会把坏结果锁死。
>   - **建议处置**：`git revert 026d502` 回到 `69c7111`（用户当时评价「bug 大致修好，只剩闪一下」）。
>     **闪动是次要问题，不应为它牺牲"能删除 / 不累计"。** 待用户指示再动。
>   - **✅ 处置已完成（2026-09-17 11:07，用户批准回退）**：`extension/lib/content/x/fill.ts` 已从
>     `69c7111` 恢复，`026d502` 的写入改动全部撤销。回退方式 = **恢复文件 + 新建提交**（未重写历史、
>     未用 reset/rebase/force）；回退前的 HEAD 已打备份 ref **`backup-2026-09-17-pre-revert`**，可随时找回。
>     **闪动问题回到「未解决」状态 —— 不再为它单独改动写入链路。**
